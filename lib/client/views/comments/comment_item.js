...
Template.telescope_comment_item.helpers({
  ...
  child_comments: function () {
    ...
    return Telescope_Comments.find({parent: this._id });
  },
  author: function () {
...
Template.telescope_comment_item.events({
  'click .queue-comment': function (e) {
    ...
    var now = new Date();
    var comment_id = Telescope_Comments.update(current_comment_id,
      {
        $set: {
  ...
  'click .sh-square-comment.upvoted': function (e, instance) {
    e.preventDefault();
    if (!Meteor.user()) {
      clearSeenErrors();
  ...
  'click .sh-square-comment.not-upvoted': function (e, instance) {
    e.preventDefault();
    if (!Meteor.user()) {
      clearSeenErrors();
      throwError("Please log in first");
