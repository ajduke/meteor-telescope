...
Template.telescope_post_edit.created = function(){
  post = Telescope_Posts.findOne(this.data.postId);
}

Template.telescope_post_edit.helpers({
  post: function () {
    return Telescope_Posts.findOne(this.postId);
  },
  created: function(){
  ...
  categories: function(){
    var post = this;
    return Telescope_Categories.find().map(function(category) {
      category.checked = _.contains(_.pluck(post.categories, '_id'), category._id) ? 'checked' : '';
      return category;
    });
  },
  categoriesEnabled: function(){
    return Telescope_Categories.find().count();
  },
  isApproved: function(){
...
Template.telescope_post_edit.events({
  'click input[type=submit]': function(e, instance){
    ...
    $('input[name=category]:checked').each(function() {
      ...
      if(category = Telescope_Categories.findOne(categoryId))
    ...
    Telescope_Posts.update(post._id,{
      $set: properties
    }, function(error){
      if(error){
        console.log(error);
        ...
        Router.go('/'+baseUrl+"/posts/"+post._id);
      }
    });
  },
