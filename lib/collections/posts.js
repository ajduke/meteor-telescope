Telescope_Posts = new Meteor.Collection('telescopePosts');

STATUS_PENDING=1;
STATUS_APPROVED=2;
STATUS_REJECTED=3;

Telescope_Posts.deny({
...
Meteor.methods({
  post: function(post){
    var headline = cleanUp(post.headline),
        ...
        postWithSameLink = Telescope_Posts.findOne({url: post.url}), // TODO: limit scope of search to past month or something
        timeSinceLastPost=timeSinceLast(user, Telescope_Posts),
        numberOfPostsInPast24Hours=numberOfItemsInPast24Hours(user, Telescope_Posts),
    ...
    if (!user || !canPost(user))
      throw new Meteor.Error(601, 'You need to login to post new stories.');
    ...
    postId = Telescope_Posts.insert(post);
  ...
  clickedPost: function(post, sessionId){
    ...
    if(_.where(clickedPosts, click).length == 0){
      ...
      Telescope_Posts.update(post._id, { $inc: { clicks: 1 }});
    }
  },
  deletePostById: function(postId) {
    ...
    var post = Telescope_Posts.findOne({_id: postId});
