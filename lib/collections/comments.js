Telescope_Comments = new Meteor.Collection('telescopeComments');

Telescope_Comments.deny({
...
Meteor.methods({
  comment: function(postId, parentCommentId, text){
    var user = Meteor.user(),
        post=Telescope_Posts.findOne(postId),
        postUser=Meteor.users.findOne(post.userId),
        timeSinceLastComment=timeSinceLast(user, Telescope_Comments),
        ...
        properties={
          ...
          'commentExcerpt': trimWords(cleanText,20),
          'postId': postId,
    ...
    var newCommentId=Telescope_Comments.insert(comment);
    ...
    Telescope_Posts.update(postId, {$inc: {comments: 1}});
    ...
    if(!this.isSimulation){
      if(parentCommentId){
        // child comment
        var parentComment=Telescope_Comments.findOne(parentCommentId);
