getSetting = function(setting, defaultValue){
  var settings=Telescope_Settings.find().fetch()[0];
  if(settings){
...
clearSeenErrors = function(){
  Telescope_Errors.update({seen:true}, {$set: {show:false}}, {multi:true});
}
t=function(message){
  var d=new Date();
}
...
getDigestURL = function(moment){
  return '/'+baseUrl+'/digest/'+moment.year()+'/'+(moment.month()+1)+'/'+moment.date()
}
...
getPostUrl = function(id){
  return Meteor.absoluteUrl()+baseUrl+'/posts/'+id;
}
getPostEditUrl = function(id){
  return Meteor.absoluteUrl()+baseUrl+'/posts/'+id+'/edit';
}
getCommentUrl = function(id){
  return Meteor.absoluteUrl()+baseUrl+'/comments/'+id;
}
getPostCommentUrl = function(postId, commentId){
  // get link to a comment on a post page
  return Meteor.absoluteUrl()+baseUrl+'/posts/'+postId+'/comment/'+commentId;
}
getCategoryUrl = function(slug){
  return Meteor.absoluteUrl()+baseUrl+'/category/'+slug;
}
...
getOutgoingUrl = function(url){
  return Meteor.absoluteUrl() + baseUrl+'/out?url=' + encodeURIComponent(url);
}
