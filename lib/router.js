...
//    pause()
  ...
  canEditPost: function (pause) {
    ...
    var post = Telescope_Posts.findOne(this.params._id);
  ...
  canEditComment: function (pause) {
    ...
    var comment = Telescope_Comments.findOne(this.params._id);
...
//  Router.onBeforeAction(filters.isLoggedOut, {only: ['signin', 'signup']});
  ...
  Router.onBeforeAction(filters.isAdmin, {only: ['posts_pending', 'all-users', 'categories', 'toolbox', 'logs']});
...
TelescopeConfig.telescope_routes = function (appBaseUrl) {
...
// Controller for post digest
  ...
  PostsDigestController = FastRender.RouteController.extend({
    ...
    data: function() {
      ...
      digest_posts = Telescope_Posts.find(parameters.find, parameters.options).count();
      return {
        posts: Telescope_Posts.find(parameters.find, parameters.options)
      }
    }
...
// Controller for comment pages
  ...
  CommentPageController = FastRender.RouteController.extend({
    ...
    data: function () {
      return {
        comment: Telescope_Comments.findOne(this.params._id)
      }
    },
    onAfterAction: function () {
      window.queueComments = false;
...
// Controller for user pages
  ...
  Router.map(function () {
    ...
    this.route('logs', {
      ...
      data: function () {
        return Telescope_Searches.find({}, {sort: {timestamp: -1}});
      },
      fastRender: true
    });
...
TelescopeConfig.telescope_routes_server = function (appBaseUrl) {
  baseUrl = appBaseUrl === undefined ? 'forum' : appBaseUrl;
  appBaseUrl = appBaseUrl === undefined ? 'forum' : appBaseUrl;
  ...
  Router.map(function () {
    ...
    this.route('out', {
      ...
      action: function () {
        ...
        if (query.url) {
          var post = Telescope_Posts.findOne({url: query.url});
          if (post) {
            Telescope_Posts.update({_id: post._id}, {$inc: {clicks: 1}});
          }
