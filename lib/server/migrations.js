...
Meteor.startup(function () {
 ...
 // migration updatePostStatus: make sure posts have a status
  if (!Migrations.findOne({name: "updatePostStatus"})) {
    ...
    console.log("//----------------------------------------------------------------------//")
    Telescope_Posts.find({status: {$exists : false}}).forEach(function (post) {
      Telescope_Posts.update(post._id, {$set: {status: 2}});
 ...
 // migration updateCategories: make sure categories have slugs
  if (!Migrations.findOne({name: "updateCategories"})) {
    ...
    console.log("//----------------------------------------------------------------------//")
    Telescope_Categories.find().forEach(function (category) {
      if(typeof category.slug === "undefined"){
        var slug = slugify(category.name);
        Telescope_Categories.update(category._id, {$set: {slug: slug}});
  ...
  if (!Migrations.findOne({name: "updatePostCategories"})) {
    ...
    Telescope_Posts.find().forEach(function (post) {
      ...
      _.each(oldCategories, function(value, key, list){
        ...
        if((typeof value === "string") && (category = Telescope_Categories.findOne({name: value}))){
      ...
      if(updating){
        ...
        Telescope_Posts.update(post._id, {$set: {categories: newCategories}});
      }
  ...
  if (!Migrations.findOne({name: "updateUserProfiles"})) {
    ...
    allUsers.forEach(function(user){
      ...
      // update postCount
      var postsByUser = Telescope_Posts.find({userId: user._id});
      Meteor.users.update(user._id, {$set: {postCount: postsByUser.count()}});
      
      // update commentCount
      var commentsByUser = Telescope_Comments.find({userId: user._id});
